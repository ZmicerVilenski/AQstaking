...
library Checkpointing {
    ...
    uint256 constant MAX_UINT192 = uint256(2**192 - 1);
    uint256 constant MAX_UINT64 = uint256(2**64 - 1);
    ...
    function add192(
        ...
        if (
            ...
            self.history[self.history.length - 1].time < time
        ) {
            self.history.push(Checkpoint(time, value));
        } else {
            Checkpoint storage currentCheckpoint = self.history[
    ...
    function get192(History storage self, uint64 time)
        internal
        view
        returns (uint192)
    {
    ...
    function get(History storage self, uint256 time)
        internal
        view
        returns (uint256)
    {
